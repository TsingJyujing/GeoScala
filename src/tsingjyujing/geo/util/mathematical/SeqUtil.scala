...
object SeqUtil {
    ...
    def movingMap[T, O](sequenceData: Iterable[T], windowSize: Int, mapFunction: List[T] => O): Iterable[O] = {
        val returnSequence = new mutable.MutableList[O]
        val itor = sequenceData.iterator
        val tempQueue = new mutable.Queue[T]
        for (_ <- 1 to (windowSize - 1)) {
            tempQueue.enqueue(itor.next())
        }
        while (itor.hasNext) {
            tempQueue.enqueue(itor.next())
            returnSequence += mapFunction(tempQueue.toList)
            tempQueue.dequeue()
        }
        returnSequence
    }
